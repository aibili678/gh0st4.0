		...
		case 2:			  
			...
			{				 
                ...
                if(RegEnumValue(hKey,dwIndex++,KeyName,&KnSize,NULL,&Type,(LPBYTE)ValueSz,&szSize) != ERROR_SUCCESS)
					break;
				char Format[]={'%','-','2','4','s',' ','%','-','1','5','s',' ','%','s',' ','\r','\n','\0'};
				char Format1[]={'%','-','2','4','s',' ','%','-','1','5','s','\0'};
				char Format2[]={'%','-','2','4','s',' ','%','-','1','5','s',' ','0','x','%','x','(','%','d',')','r','\n','\0'};
				switch(Type)				 				
				{				     
				case REG_SZ:
					wsprintf(ValueTemp,Format,KeyName,"REG_SZ",ValueSz);
					break;
				case REG_EXPAND_SZ:                   						 
					wsprintf(ValueTemp,Format,KeyName,"REG_EXPAND_SZ",ValueSz);
					break;
				case REG_DWORD:
					wsprintf(ValueTemp,Format2,KeyName,"REG_DWORD",ValueSz,int(ValueSz));
					break;
				case REG_MULTI_SZ:
                    wsprintf(ValueTemp,Format1,KeyName,"REG_MULTI_SZ");
					break;
			    case REG_BINARY:
					wsprintf(ValueTemp,Format1,KeyName,"REG_BINARY");
					break;
				}
